...
void onTimeZoneSelection(
    ...
    if (timeZoneInfo.find(timeZone) != timeZoneInfo.end()) {
        ...
        std::string response = "Вы выбрали " + city +
            " (UTC " + (offset >= 0 ? "+" : "-") +
            std::to_string(abs(offset)) + ")";

        bot.getApi().sendMessage(query->message->chat->id, response);
    } else {
        bot.getApi().sendMessage(query->message->chat->id, "Неопределенный часовой пояс");
    }
}
